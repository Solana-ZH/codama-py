...
export function getRenderMapVisitor(options: GetRenderMapOptions = {}) {
  ...
  //console.log("render",render,globalScope)
              ...
              visitAccount(node) {
                  ...
                  visitRoot(node, { self }) {
                      ...
                      if (pdasToExport.length > 0) {
                          renderMap=addToRenderMap(renderMap,'pdas/index.py', render('pdasIndex.njk', ctx));
                      }
                      if (accountsToExport.length > 0) {
                          renderMap=addToRenderMap(renderMap,'accounts/__init__.py', render('accountsIndex.njk', ctx));
                      }
                      if (instructionsToExport.length > 0) {
                          renderMap=addToRenderMap(renderMap,'instructions/__init__.py', render('instructionsIndex.njk', ctx));
                      }
                      if (definedTypesToExport.length > 0) {
                          renderMap=addToRenderMap(renderMap,'types/__init__.py', render('definedTypesIndex.njk', ctx));
                      }
                      {
                          renderMap=addToRenderMap(renderMap,'shared/__init__.py', render('sharedIndex.njk', ctx));
                          renderMap=addToRenderMap(renderMap,'shared/extension.py', render('extension.py', ctx));
                      }
                      {
                          const imports = new ImportMap().add('solders.pubkey', 'Pubkey');
                          renderMap = addToRenderMap(renderMap,
                              `program_id.py`,
                      ...
                      return mergeRenderMaps([
                          renderMap,
                          createRenderMap('__init__.py', render('rootIndex.njk', ctx)),
